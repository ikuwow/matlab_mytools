...
%　変更
if (~quiet) disp(sprintf('Solving a problem of size (m=%d, n=%d), with lambdalength=%.5e',...
            m,n,length(lambda))); end
%if (~quiet) disp('-----------------------------------------------------------------------------');end
...
for ntiter = 0:MAX_NT_ITER
    ...
    minAnu = min(At*nu);
    % ここを変更
    if (minAnu < -min(lambda))
        nu = nu*lambda/(-minAnu);
    end
    % 変更。1ノルムを計算する前にlambdaを。
    pobj  =  z'*z+sum(lambda*x,1); %||Ax-y||_2+lambda*x
